...
## opencode でモデルが選べない / 課金の警告が出る

このリポジトリの `opencode.json` で無料の **Big Pickle**（`opencode/big-pickle`）を指定しています。
`/models` で別のモデルに切り替えると、**無料表示のないモデルは課金対象**になります。
迷ったら `/models` で `big-pickle` に戻してください。

Big Pickle は「期間限定の無料提供」とされているモデルです。当日使えなくなっていた場合は、
`/models` で他の無料モデル（`Free` と付いているもの）に切り替えてください。
それも無ければスタッフに申し出てください（代替手順を用意しています）。

## `/connect` でつまずく

1. `opencode` を起動して `/connect` と入力
2. 一覧から **opencode** を選ぶ
3. ブラウザで https://opencode.ai/auth を開いてサインイン → API キーをコピー
4. opencode の入力欄に貼り付け

キーを貼り間違えた場合は、もう一度 `/connect` からやり直せます。
